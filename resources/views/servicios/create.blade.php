        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <div class="max-w-2xl mx-auto p-8 rounded">
                    <h1 class="text-2xl font-bold text-center mb-6 text-gray-800">Crear Nuevo Servicio</h1>
                    ...
                    <form action="{{ route('servicios.store') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="descripcion" id="descripcion" rows="4"
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" required>{{ old('descripcion') }}</textarea>
                        </div>
                                ...
                                @foreach ($categorias as $categoria)
                                    <div class="flex items-center">
                                        <input type="checkbox" name="categorias_ids[]" id="categoria_{{ $categoria->id }}" value="{{ $categoria->id }}"
                                               {{ in_array($categoria->id, old('categorias_ids', [])) ? 'checked' : '' }}
                        ...
                        <div class="flex justify-end">
                            <button type="submit"
                                class="bg-indigo-600 text-black px-6 py-2 rounded hover:bg-indigo-700 font-semibold">Crear Servicio</button>
                        </div>
                    </form>
